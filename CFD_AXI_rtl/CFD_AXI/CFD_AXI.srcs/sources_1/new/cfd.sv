...
module cfd #( IN_WIDTH               = 12  ,
              OUT_WIDTH              = 16  ,
              PIPE_DLY               = 142 ,
              FPGA_CLOCKS_PER_SAMPLE = 16  ,  // Number of FPGA clocks in one ADC period
              LHC_PERIOD_NS          = 25  ,
              SCALE_FACTOR_WIDTH     = 12  ,  // [INFO] Probably it will remain hardcoded
              T_GATE_DELAY_NS        = 2.5
           )( 
              //==========================================================
              // AXI WRAPPER LEVEL CONNECTION
              //==========================================================
              input  logic                          clk              ,
              input  logic                          rst_p            ,
              ...
              input  logic                          trigger          , // it seems like it's the same signal as axi_data_vld but it isn't!!!
              output logic                          cfd_rdy          ,
              input  logic                          axi_data_vld     , // unused
              //==========================================================
              // AXI LITE CONNECTION
              //==========================================================
...
localparam PIPE_DLY_INT           = PIPE_DLY*FPGA_CLOCKS_PER_SAMPLE;
...
logic [             1:0] COUNTER_CTRL_LATENCY;
...
zc_interpolator #(.FPGA_CLOCKS_PER_SAMPLE(FPGA_CLOCKS_PER_SAMPLE),
                 ...
                 .result            (zc_result      ),
                 .result_vld        (zc_result_vld  ), // it will replace zc_result_vld_d, inside its still zc_result_vld_d
                 .result_vld_d      (zc_result_vld_d)  // inside module its zc_result_vld
);
...
passthrough #(
  ...
  .passth_th_sample_out (passthrough_th_out   )
);


assign COUNTER_CTRL_LATENCY = 2'b11; // [WARNING] counter should stop counting 3 clks before it really stops,
                                     //           poor solution...

assign out_clocks_scaled = out_clocks << ZC_OUT_FRACT;

// result = zc_result + counter - counter latency (scaled to proper fxp formats)
assign result = (result_proc_cntrl==1'b1) ? zc_result + out_clocks_scaled - {COUNTER_CTRL_LATENCY, {ZC_OUT_FRACT{1'b0}}}: '0;

assign pulse_out = zc_result_vld_d; // & result_proc_cntrl;
assign data_out  = result; //zc_result;
assign th_passthrough_out_vld = passthrough_th_out;
assign cfd_rdy = 1'b1;
